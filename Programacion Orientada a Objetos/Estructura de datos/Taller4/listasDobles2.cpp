...
listaDoble::nodo* listaDoble::contains(int val1, int val2){
	...
	if(isEmpty())
		return NULL;
	while(tmp -> pSgte != NULL){
		if((tmp -> valor1 == val1 && tmp-> valor2 == val2) or (tmp -> valor1 == val2 && tmp-> valor2 == val1))
			return tmp;
		tmp = tmp -> pSgte;
	}
	if(tmp -> pSgte == NULL && (tmp -> valor1 == val1 && tmp-> valor2 == val2) or (tmp -> valor1 == val2 && tmp-> valor2 == val1))
		return tmp;
	return NULL;
}
