...
void imprimirArreglo(int arreglo[], int tam){
 for(int i=0; i < tam; i++){
 	cout <<"jugador "<< i << " tiene: " << arreglo[i]<< endl; 
 }
}
void llenarListaAleatorio(listaDoble &lst){
	int var1=0;
	int var2=0;
	int cont=0;
	while (cont < 28){
		var1 = rand()%7;
		var2 = rand()%7;
		if(lst.contains(var1, var2)== NULL){
...
bool fichaValida(listaDoble &jugador, listaDoble &juego){
	...
	//cout << P1 << F2 << endl;
	for(int i = 6; i >= 0; i--){
		if((jugador.contains(i, P1)!= NULL) || (jugador.contains(F2, i) != NULL))
			return true;
	}
	return false;
}
...
int sumaFichas(listaDoble jugador){
	...
	while(!jugador.isEmpty()){
		cont = cont + jugador.retornarFirst1() + jugador.retornarFirst2();
		jugador.removeFirst();
	}
	return cont;
}
...
void llenarDineroJugadores(int dineroJugadores[], int cantJ){
	for(int i=0; i < cantJ; i++){
		dineroJugadores[i]=2000;
	}
}

void llenarFichasJugadores(Cola &fichasTotal,int jugadores[] ,int cantJ){
	for(int i=0; i < cantJ; i++ ){
		llenarFichasJugador(fichasTotal, jugadores[i]);
	}
}
